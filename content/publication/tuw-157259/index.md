...
abstract: 'Building Models or Building Information Modells (BIM) are fine-grained,
  ...
  access to local and remote client appliciations. <br> Existing software architectures
  ...
  but potentially massive check-in/check-out operations on a central model repository.<br>
  ...
  to building systems for client applications.<br> The main challenge for such operation-phase
  ...
  not designed for high concurrency.<br> At the core of the proposed architectrue,
  the building obejct model is implemented in a bitemporal main meomry database with
  persistent backing storage. Changes to model obejcts are versioned, allowing simple
  and transparent access to a full model history ("time travel"). The database ensures
  serializable transactions with a high degree of concurrency using a variant of the
  multi-version transaction ordering (MVTO) algorithm. Transactions amay be prioritised
  to ensure low latency updates. The performance and scalability of the concurrency
  control mechanism compares favourably with a simple locking scheme. The model server
  provides full support for happened-before and known-before ordering of date even
  when the data arrive unordered or very late by allowing retroactive insertion of
  records.<br> Distribution design is one of the key points in the software architecture:
  instead of using a reference architecture (such as client-server) and working around
  its limitations, the moste beneficial runtime locations of tasks and data for given
  requirements are considered. This is particularly important for client appliciations.<br>
  ...
  can be installed and activated at run-time without service disruption.<br> Batch
  behaviour is characterised by intensive, long-running work on a fixed set of model
  objects, as is typical for building simulation and other analysis tasks. Write operations
  ...
  degree and allows simple load distribution with a minimum of central administration.<br>
...
date: '2007-01-01'
featured: false
publication: '*The Design of a Building Model Service*'
publication_types:
- '7'
